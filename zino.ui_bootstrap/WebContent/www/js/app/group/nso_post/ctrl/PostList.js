define([
    'text!group/nso_post/tmpl/Post_List.html', 
    'text!group/nso_post/tmpl/Post_List_Element.html', 
    ],
    function(
		Post_List,
		Post_List_Element
	...
	const PostList = function (grpName, header, content, footer) {
		...
		const pr_SV_LST				= "SVNsoPostLstByEntIdDeal";
		const pr_SV_NEW_COMMENTS	= "SVNsoPostNewDeal";
		...
		const do_lc_get_list_ByAjax_Dyn = (id, reBuild = false) => {
			const cond 			= {
					entId: id				, entTyp : pr_ID_TABLE_OFFER, reBuild : true,
					number: pr_POST_NUMBER	, nbLevel: pr_POST_HAS_SUB	, forced: true, 
			};
			
			const ref 			= req_gl_Request_Content_Send_With_Params(pr_SERVICE_CLASS, pr_SV_LST, cond);
		...
		const do_lc_send_comment = (comment, iParent, id) => {
			const ref 		= req_gl_Request_Content_Send_With_Params(pr_SERVICE_CLASS, pr_SV_NEW_COMMENTS, {comment, id, iParent});
