...
//			App.path.BASE_URL_DNS = "http://localhost:8084"
			...
			var urlAPI = window.location.origin + App.path.BASE_URL_API_CHAT;
//			var host = window.location.host;
			
//			var	  authStr		= "://user:user@";
...
//			const urlAPI 		= App.path.BASE_URL_API + "/chat";
//			const urlAPI 		= App.path.BASE_URL_DNS + App.path.BASE_URL_API;
			
			var	  authStr		= "://";
			const urlSocket 	= urlAPI.replace("http","ws").replace("://", authStr);
			...
			try{
				...
				urlConn 		= urlSocket + "/" + username + "/" + tok +"/";
...
//			}
		...
		this.do_gl_init_SocketChat = function(username){
			setTimeout(() => {
		...
		this.do_gl_closeSocket = function(){
			pr_socket_status = 0;
			if (pushSocket) {
				pushSocket.close();
				pushSocket = null;
		...
		const do_lc_push_new_user_read = (msgContent) => {
			if(VIEW_PART ===  App.router.part.PRJ_CHATROOM){
				...
				App.controller.ChatRoom.Chat.do_lc_new_chatroom_user_read(msg.user, msg.idMsg);
			}
		}
		...
		const do_lc_push_validate_group = (msgContent) => {
			...
			if(VIEW_PART ===  App.router.part.PRJ_CHATROOM){
				App.controller.ChatRoom.Person.do_lc_push_notif_validate_socket(msgContent);
			} else {
				$("#span_new_msg").removeClass("hide");
			}
		}
		...
		const do_lc_push_not_validate_group = (msgContent) => {
			...
			if(VIEW_PART ===  App.router.part.PRJ_CHATROOM){
				App.controller.ChatRoom.Person.do_lc_push_notif_not_validate_socket(msgContent);
			} else {
				$("#span_new_msg").removeClass("hide");
			}
		}
		...
		const do_lc_push_list_online = (list) => {
			...
			if(lstUserOnline && lstUserOnline.length){
				...
				App.controller.ChatRoom.Person.do_lc_push_new_list_online();
		...
		var do_lc_handle_RTCMsg= async function (msg) {
			...
			if (msg.typ === 'candidate') {
				...
				if (pr_inVideoCall){
					...
					App.controller.ChatRoom.ChatWebRTC.do_lc_show_popup_new_call(uId, msg);
				}
