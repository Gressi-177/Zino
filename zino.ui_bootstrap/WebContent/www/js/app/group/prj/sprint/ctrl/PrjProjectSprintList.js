define([
	...
	const PrjProjectSprintList = function (grpName, header, content, footer) {
		...
		const pr_SV_TASK_LST		= "SVTaskList";
		...
		this.do_lc_init	= function(){
			...
			pr_ctr_PrjSprint_Main        	=   App.controller.PrjSprint.Main;
			pr_ctr_PrjSprint_List        	=   App.controller.PrjSprint.List;
			...
			tmplName.PRJ_SPRINT_LIST_TAB		= "PrjSprint_List_Tab";
			tmplName.PRJ_SPRINT_LIST_CONTENT	= "PrjSprint_List_Content";
			tmplName.PRJ_SPRINT_POPUP_NEW    	= "PrjSprint_Popup_New";
		...
		this.do_lc_show = function(pId, pCode){               
			try{
				...
				if(pr_GROUP && pr_CODE) do_lc_get_prj(pr_GROUP, pr_CODE);

				do_lc_load_view();
				do_get_list_ByAjax();
				do_binding_event();
			}catch(e) {				
				console.log(e); //do_gl_send_exception(App.path.BASE_URL_API_PRIV, pr_ctr_Main.var_lc_URL_Header, App.network, "prj.project", "PrjProjectSprintList", "do_lc_show", e.toString()) ;
			}
		};

		const do_lc_load_view = () => {	
			$("#div_task_content").html(tmplCtrl.req_lc_compile_tmpl(tmplName.PRJ_SPRINT_LIST_TAB, {grp:pr_GROUP, code: pr_CODE}));
		}
		
		const do_get_list_ByAjax = (forced) => {	
			let dataSend			= {isAll: true, typ00: pr_TYP00_SPRINT,  group: pr_GROUP, searchKey: pr_SEARCHKEY, searchUser: (pr_SEARCHKEY_USER && pr_SEARCHKEY_USER.length) ? pr_SEARCHKEY_USER : null, wA : false, wParent: true,  forced};
		...
		const do_binding_event = () => {
			...
			$(".btn_new_sprint").off("click").on("click", function(){
				...
				App.MsgboxController.do_lc_show({
					...
					content 	: tmplCtrl.req_lc_compile_tmpl(tmplName.PRJ_SPRINT_POPUP_NEW, {lev: pr_LEVEL_PRJ, currencys: App.data.currencys, prj: pr_PROJECT}),	
					autoclose	: false,
					buttons		: {
						OK: {
		...
		const do_lc_init_element = obj => {
			...
			let option2		= {
					fileinput	: { 
						parallelUploads	: 10,
			            uploadMultiple	: true,
						param 			: {typ01: 2, typ02: 10}},//option here
					obj			: obj//file existing here
			}
