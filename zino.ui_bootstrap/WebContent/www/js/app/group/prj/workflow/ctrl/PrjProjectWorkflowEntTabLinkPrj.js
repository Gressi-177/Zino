define([
        ...
        'text!group/prj/workflow/tmpl/PrjWorkflow_Ent_Tab_Link_Prj.html',

	],
	function(	
            PrjProject_Ent_Tab_Link_Prj
	){
    ...
    var PrjProjectWorkflowEntTabLink 	= function (grpName, header, content, footer) {
        ...
        tmplName.PRJ_PROJECT_ENT_TAB_LINK_PRJ = "PrjProject_Ent_Tab_Link_Prj";
        ...
        var do_lc_load_view = function(){
            ...
            tmplCtrl				.do_lc_put_tmpl(tmplName.PRJ_PROJECT_ENT_TAB_LINK_PRJ			, PrjProject_Ent_Tab_Link_Prj);
        }

        this.do_lc_get_list_member = function(prj){
            let ref 		= req_gl_Request_Content_Send_With_Params("ServicePrjProject", "SVEpicGet", {id: prj.id});	
        ...
        var do_lc_bind_event_members_prj = function(members, idPrj){
            ...
            $("#btn_add_link_prj").off("click").on("click", function(){
                $(".action-item-link-prj").removeClass("hide");
                $(this).addClass("hide");
            })

            $("#a_btn_save_link_prj").off("click").on("click", function(){
                do_lc_save_member_prj(members, idPrj);
            })

            $("#a_btn_cancel_link_prj").off("click").on("click", function(){
                do_lc_show_prj_member(members, idPrj);
            })

            $(".link-prj-edit").off("click").on("click", function(){
                let $this 			= $(this);
                let {memid} 		= $this.data();
            ...
            $(".link-prj-delete").off("click").on("click", function(){
                let {memid} = $(this).data();
                let mem 	= pr_MEM_TEMP[memid];
                if(mem){
            ...
            let el = "#inp_list_prj";
            let reqSelectPrj = function(event, item){
                ...
                $("#tabLinkPrj table tbody").append(selOpt);
                do_lc_bind_event_autocomplete(pr_MEM_TEMP);
                $(el).blur().val("");
            }

            let options = {
                    dataService : ["ServicePrjProject", "SVLstSearch"], fSelect: reqSelectPrj, customShowList: do_lc_customLst_item_autocomplete
            }
        ...
        var do_lc_save_member_prj = function(members, idPrj){
            let ref 		= req_gl_Request_Content_Send_With_Params("ServicePrjProject", "SVEpicSave", {idPrj, members: JSON.stringify(Object.values(pr_MEM_TEMP))});	
        ...
        var do_lc_show_prj_member = function(members, idPrj){
            do_lc_load_view();
            $("#div_prj_link_prj")	.html(tmplCtrl.req_lc_compile_tmpl(tmplName.PRJ_PROJECT_ENT_TAB_LINK_PRJ			, members));
