define([
        'jquery',
        'text!group/user_job/report/tmpl/JobReport_Ent_Tab_Report_Detail.html',
        'text!group/user_job/report/tmpl/JobReport_Detail_Action.html',
		'text!group/user_job/report/tmpl/JobReport_Ent_Tab_Report_Resume.html',
		'text!group/user_job/report/tmpl/JobReport_Ent_Tab_Report_Resume_Line.html',
        ],
        function($,
				...
				if(weekday != "HLD"){
					if(pr_mode == pr_ctr_Main.var_lc_MODE_NEW || pr_mode == pr_ctr_Main.var_lc_MODE_MOD || pr_mode == pr_ctr_Main.var_lc_MODE_SEL){
						...
						$(nTd).on("click", function(){
							...
							if(pr_mode != pr_ctr_Main.var_lc_MODE_NEW )$("#a_btn_save_detail_In2, #a_btn_cancel_detail_In2")	.removeClass("hide");
			...
			if(pr_mode == pr_ctr_Main.var_lc_MODE_MOD || pr_mode == pr_ctr_Main.var_lc_MODE_NEW || pr_mode == pr_ctr_Main.var_lc_MODE_SEL) {
				...
				if(row.length == maxiRow){
					...
					$("#btn_add").on("click", function(){
						if(pr_mode != pr_ctr_Main.var_lc_MODE_NEW){
							$("#a_btn_save_detail_In2, #a_btn_cancel_detail_In2")	.removeClass("hide");
						}
						do_lc_bind_event();
					});
