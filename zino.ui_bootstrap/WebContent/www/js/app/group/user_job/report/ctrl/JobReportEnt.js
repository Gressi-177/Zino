define([
        'jquery',
        'text!group/user_job/report/tmpl/JobReport_Ent.html'
        ],
        function($,
        		JobReport_Ent) {
	...
	var JobReportEnt     = function (grpName, header, content, footer) {
		...
		var pr_SV_GET				= "SVJobReportGet"; 
		var pr_SV_NEW				= "SVJobReportNew"; 
		var pr_SV_DEL				= "SVJobReportDel"; 
		
		var pr_SV_MOD				= "SVJobReportMod"; 	//if not use lock
				
		var pr_SV_LCK_NEW			= "SVJobReportLckReq"; 
		var pr_SV_LCK_END			= "SVJobReportLckEnd"; 
		var pr_SV_LCK_DEL			= "SVJobReportLckDel";
		
		var pr_SV_WD				= "SVJobReportWorkingDate";
		//------------------controllers------------------------------------------------------
		...
		//---------------------------------------------------------------------------------------------
		this.do_lc_Save_Entity = function (divContent, ent, mode){
			if (!divContent) divContent	= pr_divContent;
			...
			let existP = do_check_exist_project_report(data.data);
			if(existP){
				do_gl_show_Notify_Msg_Error($.i18n("job_report_msg_rp_cant_submit"));
				return false;
			}
			
			if(!data.data.val02) data.data.val02 = parseInt($("#inp_val02").val());
			if(!data.data.val03) data.data.val03 = parseInt($("#inp_val03").val());
			
			do_lc_handle_rp_resume(data);
		...
		var do_send_data= function(data, mode){
			...
			var fError 			= req_gl_funct(App, do_gl_show_Notify_Msg_Error [$.i18n("job_report_error"), 0]);	
		...
		var do_lc_afterSave_EntContent = function(sharedJson){
			if(sharedJson[App['const'].SV_CODE] == App['const'].SV_CODE_API_YES) {
				...
				do_gl_show_Notify_Msg_Error ($.i18n('job_report_error'));
			}
		}
		...
		this.do_lc_save		= function(obj, mode, rpStat, directSave){	//save new object or save with lock	
			...
			let existP = do_check_exist_project_report(cra.data);
			if(existP){
				do_gl_show_Notify_Msg_Error($.i18n("job_report_msg_rp_cant_submit"));
				return false;
			}
			
			let existR = do_check_condition_exist_report(cra.data, directSave);
			if(existR){
			...
			if(cra.hasError) {
				...
				if (directSave){
					...
					if (rpStat == lc_rp_stat_denied || rpStat == lc_rp_stat_pending)  {
...
//								}
		...
		//---------------------------------------------------------------------------------------------
		//	DO CHECK CONDITION EXIST REPORT
		//---------------------------------------------------------------------------------------------
		
		var do_check_condition_exist_report = function(cra, directSave){
			if(directSave) return false;
			let currentCode = cra.code;
			let lstReport   = App.data.lstReport;
			for(let i=0; i< lstReport.length; i ++){
				if(lstReport[i].code == currentCode){
					return true;
				}
			}
		...
		//---------------------------------------------------------------------------------------------
		//	DO CHECK EXIST PRJOJECT REPORT
		//---------------------------------------------------------------------------------------------
		
		var do_check_exist_project_report = function(cra){
			let lstRp = cra.lstRp;
			for(let i = 0; i < lstRp.length; i ++){
				if(lstRp[i].catId01 == 0 && lstRp[i].catId01_Select.length == 0){
					$("#table_report_detail").css("border", "1px solid red");
					return true;
				}
			}
			
			$("#table_report_detail").css("border", "unset");
			return false;
		}
		...
		var do_lc_handle_rp_resume = function(cra) {
			let lstRpResume 	= cra.data.lstRpResume;
			...
			cra.data.lstRpResume = newLstResume;
		}
		...
		var do_lc_handle_rp_resume_backToList = function(cra) {
			...
			for (let i = 1; i <= 31; i++) {
				...
				if (objRpResume[attr])
					newLstResume.push({"id": objRpResume.id, "day":i, "cmt": objRpResume[attr]});
			}
		...
		var do_send_mod_direct = function(cra){
			var ref 		= req_gl_Request_Content_Send(pr_SERVICE_CLASS, 'SVJobReportModDirect');
		...
		var do_send_new_continue = function(cra) {
			...
			var fError 			= req_gl_funct(null, do_show_msg, ["KO"]);
			cra.do_lc_send_data(App.path.BASE_URL_API_PRIV, App.data["HttpSecuHeader"], ref, fSucces, fError);
		}
