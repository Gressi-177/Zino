define([
        ...
        'text!group/job/report/tmpl/JobReport_List_Report_By_User_Header.html',  
        'text!group/job/report/tmpl/JobReport_List_Report_By_User_Content.html',
		
        'text!group/job/report/tmpl/JobReport_List_Report_Content.html',
        
        'text!group/job/report/tmpl/JobReport_List_User_Header.html',  
        'text!group/job/report/tmpl/JobReport_List_User_Content.html',
	...
	var JobReportList 	= function (grpName) {
		...
		var pr_div_Rp_By_User_Header 		= "#div_JobReport_List_Report_By_User_Header";
		...
		var pr_div_Lst_User_Header 			= "#div_JobReport_List_User_Header";
			...
			try{
				...
				if(rightCode == -1){
					...
					if(rightCode == -1){
						...
						do_get_list_rp_of_user(App.data.user.id);
					}
				} else {
				...
				do_gl_send_exception(App.path.BASE_URL_API_PRIV, pr_ctr_Main.var_lc_URL_Header, App.network, "job.report", "JobReportList", "do_lc_show", e.toString()) ;
			}
		};
		...
		function do_get_list_rp_of_user(uId03){
			var ref  = req_gl_Request_Content_Send("SVLstDyn");
			ref["uId03"] = uId03;
			$(pr_div_Rp_By_User).show();
			$(pr_div_Rp_By_User_Header	).html(tmplCtrl.req_lc_compile_tmpl(tmplName.JOB_REPORT_LIST_REPORT_BY_USER_HEADER	, {}));
			$(pr_div_Rp_By_User_Content	).html(tmplCtrl.req_lc_compile_tmpl(tmplName.JOB_REPORT_LIST_REPORT_BY_USER_CONTENT, {}));
		...
		function do_get_list_rp_pending(currentRpCode){	
			...
			$(pr_div_Rp_Pending_Content	).html(tmplCtrl.req_lc_compile_tmpl(tmplName.JOB_REPORT_LIST_REPORT_PENDING_CONTENT, {}));
		...
		function do_get_list_rp_validated(currentRpCode){	
			...
			$(pr_div_Rp_Validated_Content).html(tmplCtrl.req_lc_compile_tmpl(tmplName.JOB_REPORT_LIST_REPORT_VALIDATED_CONTENT, {}));
		...
		function do_get_list_info_user(){
			var ref  = req_gl_Request_Content_Send("SVLstUserDyn");
			ref["codeRp"]  = App.data.currentReportCode;
			
			$(pr_div_Lst_User_Header	).html(tmplCtrl.req_lc_compile_tmpl(tmplName.JOB_REPORT_LIST_USER_HEADER , {}));
		...
		var do_bind_list_user = function(data, div, oTable){
			...
			$(div).find('.table-datatableDyn tbody').on('click', 'tr', function(){
				...
				do_get_list_rp_of_user(objUser.id);
