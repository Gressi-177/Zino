define([
	...
	var JobHolidayEntTab01     = function (header,content,footer) {
		...
		this.do_lc_show		= function(obj, mode){
			...
			try{
				tmplContr.do_lc_put_tmpl(tmplName.JOB_HOLIDAY_ENT_TAB_01, JobHoliday_Ent_Tab_01); 			
				$(pr_divContent).html(tmplContr.req_lc_compile_tmpl(tmplName.JOB_HOLIDAY_ENT_TAB_01, obj));
				
				do_bind_event (obj, mode);
			}catch(e) {				
				do_gl_send_exception(App.path.BASE_URL_API, pr_ctr_Main.var_lc_URL_Header, App.network, "job.holiday", "JobHolidayEntTabGeneral", "do_lc_show", e.toString()) ;
			}
		};
