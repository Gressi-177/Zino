define([
	...
	var JobHolidayEnt     = function (header,content,footer) {
		...
		this.do_lc_show		= function(obj, mode){			
			...
			tmplContr.do_lc_put_tmpl(tmplName.JOB_HOLIDAY_ENT	, JobHoliday_Ent); 		
			$(pr_divContent).html(tmplContr.req_lc_compile_tmpl(tmplName.JOB_HOLIDAY_ENT, {mode : pr_ctr_Main.var_lc_MODE_INIT}));
			...
			if (!obj){
				...
				$(pr_divContent).html(tmplContr.req_lc_compile_tmpl(tmplName.JOB_HOLIDAY_ENT, obj));				
		...
		var do_send_new_continue = function(data) {
			...
			data.do_lc_send_data(App.path.BASE_URL_API_PRIV, pr_ctr_Main.var_lc_URL_Aut_Header, ref, fSucces, fError, "obj");
		}
		...
		var do_send_mod_continue = function(data) {
			...
			data.do_lc_send_data(App.path.BASE_URL_API_PRIV, pr_ctr_Main.var_lc_URL_Aut_Header, ref, fSucces, fError);
		}
