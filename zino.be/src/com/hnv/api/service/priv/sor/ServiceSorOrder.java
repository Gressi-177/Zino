...
public class ServiceSorOrder implements IService {
	...
	public static final String SV_CLASS 				= "ServiceSorOrder".toLowerCase();	
	...
	public void doService(JSONObject json, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if(sv.equals(SV_LCK_REQ)					&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_SOR_ORDER_GET)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLckReq(user, json, response);
			} else if(sv.equals(SV_LCK_SAV)				&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_SOR_ORDER_MOD)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLckSav(user, json, response);
			} else if(sv.equals(SV_LCK_END)				&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_SOR_ORDER_MOD)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLckEnd(user, json, response);		
			} else if(sv.equals(SV_LCK_DEL)				&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_DEL, APIAuth.R_SOR_ORDER_DEL)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLckDel(user, json, response);
				
			} else if(sv.equals(SV_NEW)					&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_NEW, APIAuth.R_SOR_ORDER_NEW)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doNew(user, json, response);
				
			} else if(sv.equals(SV_NEW_VENDOR)			&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_NEW, APIAuth.R_SOR_ORDER_NEW)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doNewFromVendor(user, json, response);
				
			} else if(sv.equals(SV_MOD)					&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_SOR_ORDER_MOD)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doMod(user, json, response);

			} else if(sv.equals(SV_MOD_STAT)			&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_MOD, APIAuth.R_SOR_ORDER_MOD)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doModStatDetail(user, json, response);	

			} else if(sv.equals(SV_DEL)					&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_DEL, APIAuth.R_SOR_ORDER_DEL)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
			...
			} else if(sv.equals(SV_GET)					&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_SOR_ORDER_GET)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
					doGet(user, json, response);
			} else if(sv.equals(SV_LST_DYN)				&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_SOR_ORDER_GET)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLstDyn(user, json, response);

			} else if(sv.equals(SV_LST)					&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_SOR_ORDER_GET)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLst(user, json, response);


			}  else if(sv.equals(SV_LST_DYN_BY_PERSON)	&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_SOR_ORDER_GET)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
				doLstDynByPerson(user, json, response);

			} else if(sv.equals(SV_LST_REP_PERSON)		&& ( APIAuth.canAuthorizeWithOneRight(user, APIAuth.R_ADMIN, APIAuth.R_AUT_ALL_GET, APIAuth.R_SOR_ORDER_GET)
														||	 APIAuth.canAuthorize(user, SV_CLASS, sv))){
	...
	private static void doNewAlarmPriceAcceptStock(TaAutUser user, TaSorOrder order) throws Exception {
		...
		List<TaTpyInformation> listAlarm = TaTpyInformation.DAO.reqList_In(TaTpyInformation.ATT_I_ENTITY_ID, listIDMat, 
				Restrictions.eq(TaTpyInformation.ATT_I_TYPE_01, TaTpyInformation.TYPE_01_BASE));
					...
					for (TaMatPrice p:listPrice) {
						if (p.req(TaMatPrice.ATT_I_MAT_MATERIAL) == d.req(TaSorOrderDetail.ATT_I_MAT_MATERIAL) 
							...
							if (priceMat * (valAlarm / 100 + 1) <= priceOrder) {
								...
								newAlarm.reqSet(TaTpyInformation.ATT_I_TYPE_01, 		TaTpyInformation.TYPE_01_BASE);
