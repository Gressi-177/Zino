...
//		}
	...
	public static void doBuildAvatarForList(List<TaNsoPost>  list) throws Exception {
		...
		List<TaTpyDocument> 	docs 	= TaTpyDocument.reqTpyDocuments (DefDBExt.ID_TA_NSO_POST, ids, TaTpyDocument.TYPE_01_FILE_MEDIA, TaTpyDocument.TYPE_02_FILE_IMG_AVATAR, null, null);
			...
			if(!mapDoc.isEmpty()) {
				for (TaNsoPost p: list){
					if(mapDoc.containsKey(p.reqRef())) {
						p.reqSet(TaNsoPost.ATT_O_AVATAR, mapDoc.get(p.reqRef()));
					}
				}
			}
		}
